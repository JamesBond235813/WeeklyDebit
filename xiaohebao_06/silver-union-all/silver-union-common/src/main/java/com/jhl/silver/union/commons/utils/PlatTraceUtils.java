...
public class PlatTraceUtils {
    ...
    protected static int seqHouse = START;
    ...
    public static String genTrace(String dateFormat, String busiFlag, int totalLength) {
        ...
        //拼接流水
        String currentSeq = "";
        synchronized (PlatTraceUtils.class) {
            if (seqHouse >= LIMIT) {
                seqHouse = START;
            }
            currentSeq = "" + seqHouse;
    ...
    /**
     * 生成平台流水32位 含busiFlag长度
     *
     * @param busiFlag 业务标识
     * @return
     */
    public static String genTrace32(String busiFlag) {
        return genTrace(DEFAULT_FULL_DATE_FMT, busiFlag, 32);
    }
     ...
     * @param randomAlphanumericCnt 随机数位数
     * @return
     */
    public static String simpleTrace(long timestamp, int randomAlphanumericCnt) {
        return timestamp + RandomStringUtils.randomAlphanumeric(randomAlphanumericCnt > 0 ? randomAlphanumericCnt : 4);
    }
     ...
     * @return
     */
    public static String simpleTrace() {
        return simpleTrace(System.currentTimeMillis(), 4);
    }
