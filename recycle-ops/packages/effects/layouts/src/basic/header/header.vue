...
// function handleOnlineStatusChange(checked: boolean) {
//   emit('handleOnlineStatusChange');
// }
...
const handleOnlineStatusChange = (checked: boolean) => {
  emit('handleOnlineStatusChange', checked);
};

onMounted(() => {
  const userStore = useUserStore();
  userOnlineStatus.value = userStore.userInfo.onlineStatus === 1;
});
</script>
  ...
  <div class="flex h-full min-w-0 flex-shrink-0 items-center">
    <template v-for="slot in rightSlots" :key="slot.name">
      <slot :name="slot.name">
        ...
        <template v-else-if="slot.name === 'user-online-status'">
          <Switch
            class="mr-1 sm:mr-4"
            @change="handleOnlineStatusChange"
            v-model:checked="userOnlineStatus"
            checked-children="在线"
